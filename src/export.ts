...
export class JsonExportAdapter implements ExportAdapter {
  ...
  export(_words: Word[], state: State) {
    return new Blob([JSON.stringify({ version: 2, exportedAt: new Date().toISOString(), state }, null, 2)], { type: 'application/json' });
  }
}
...
export class AnkiTsvExportAdapter implements ExportAdapter {
  ...
  export(words: Word[], state: State) {
    ...
    const rows = words.filter(word => studied.has(word.id)).map(word =>
      [word.word, word.phonetic, word.meaning, word.category ?? '', 'lexigraph'].map(escape).join('\t')
    );
