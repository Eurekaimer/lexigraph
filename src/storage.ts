...
export const emptyState = (): State => ({ reviews: {}, mistakes: [], history: [] });
...
class LocalFileStorage implements StorageAdapter {
  ...
  async save(state: State, keymap: Keymap) {
    const response = await fetch('/api/profile', {
      ...
      body: JSON.stringify({ version: 2, updatedAt: new Date().toISOString(), state, keymap }),
    });
...
export async function createStorage(): Promise<StorageAdapter> {
  try {
    ...
    if (response.ok && (await response.json()).mode === 'local-file') return new LocalFileStorage();
  } catch { /* Pages and Vite dev mode use browser storage. */ }
