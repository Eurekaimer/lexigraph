...
export function renderStudyView(model: StudyViewModel) {
  ...
  return `<section class="study-layout">
    <aside class="session-panel card" aria-label="今日学习进度">
      ...
      <p class="panel-note">新词从五个词频层均衡抽取，让每天的学习难度更稳定。</p>
    </aside>
    ...
    <aside class="plan-panel card" aria-label="今日计划与使用提示">
      ...
      <h3>${icon("info")}使用提示</h3>
      <ul class="tip-list"><li>Space 只切换释义，不会提交评分。</li><li>误触后按 Z，恢复评分前的完整状态。</li><li><b>完全掌握</b>会移出自动复习，请谨慎选择。</li></ul>
    </aside>
  </section>`;
}
